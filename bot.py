...
if bottoken != None:
    try:
        Yuki = TelegramClient("bot", apiid, apihash).start(bot_token=bottoken)
...
@Yuki.on(events.NewMessage(incoming=True, pattern="^/start"))
async def msgg(event):
    await send_start(event, "msg")


@Yuki.on(events.callbackquery.CallbackQuery(data="bck"))
async def bk(event):
    await send_start(event, "")


@Yuki.on(events.callbackquery.CallbackQuery(data="help"))
async def send_help(event):
    await event.edit(
        "**Torrent Searcher.**\n\nSend me a query and I'll search for available magnet links!\nPowered by Torrent API, by @tprojects!\n\nJoin @Black_Knights_Unio if you liked this bot!",
        buttons=[
...
@Yuki.on(events.NewMessage(incoming=True, func=lambda e: e.is_private))
...
@Yuki.on(events.callbackquery.CallbackQuery(data=re.compile(b"det_(.*)")))
async def src(event):
...
@Yuki.on(events.callbackquery.CallbackQuery(data=re.compile(b"sr_(.*)")))
async def src(event):
    tmp = event.data_match.group(1).decode("UTF-8")
...
buttons = [
    ...
    [
        Button.url("Channel", url="https://t.me/Black_Knights_Union"),
        Button.url("Support", url="https://t.me/Black_Knights_Union_Support"),
    ],
]


async def send_start(event, mode):
    user_ = await Yuki.get_entity(event.sender_id)
    xmsg = f"Hi {user_.first_name}.\n\nI am a Torrent Searcher Robot.\nCheck the help for more!"
    if mode == "msg":
...
print("[Yuki] Torrent Searcher Robot Is Starting. | Yūki • Black Knights Union Project | Licensed Under GPLv3.")
print("[Yuki] Successfully Connected With A  Yūki • Data Center • Mumbai")
Yuki.run_until_disconnected()
